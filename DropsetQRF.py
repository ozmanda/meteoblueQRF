...
class DropsetQRF:
    def __init__(self, datasets: dict, confidence_interval=None):
        """
        Class for QRF error estimation using the "dropset" method, similar to the Leave-One-Out Cross-Validation, or a
        k-wise cross-validation with k=1.
        :param datasets: dropset dataset, delivered as a dictionary containing a DataFrame for each station
        :param confidence_interval: confidence interval for testing, must be between 0 and 1
        """
        self.Output: dict = {}
        self.station_summary: dict = {'Station': [], 'MSE': [], 'Standard Deviation': [], 'Mean Error': []}
